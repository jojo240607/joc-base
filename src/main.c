...
static void app_main_task(void *arg)
{
    ...
    log_printf(app_log(), LOG_INFO, "main", "READY. Commands: PING / ECHO <text> / BIST / ADC [ch] / TEMP / TICKS / I2C_IRQ / USBOPEN / USBCLOSE / USBSTAT / USBDBG [0|1] / RTOS / RTOSIPC / RTOSBUS / RTOSMPU / RTOSSTRESS / RTOSFPU / RTOSBH / RTOSP4 / RTOSUSR / RTOSRR / RTOSKOBJ / RTOSALL\n");
    ...
    while (1)
    {
        ...
        if (c == '\r' || c == '\n')
        {
            if (idx > 0)
            {
                ...
                else if (strcmp(line, "RTOSRR") == 0)
                {
                    int rrok = rtos_rr_selftest();
                    char out[32];
                    int n = snprintf(out, sizeof(out), "RTOSRR %s\r\n", rrok ? "PASS" : "FAIL");
                    g_console->vtable->write(g_console, out, (size_t)n);
                }
                ...
                else if (strcmp(line, "RTOSP4") == 0)
                {
                    int p4ok = rtos_p4_selftest();
                    char out[32];
                    int n = snprintf(out, sizeof(out), "RTOSP4 %s\r\n", p4ok ? "PASS" : "FAIL");
                    g_console->vtable->write(g_console, out, (size_t)n);
                }
                ...
                else if (strcmp(line, "RTOSBH") == 0)
                {
                    int bhok = rtos_bh_selftest();
                    char out[32];
                    int n = snprintf(out, sizeof(out), "RTOSBH %s\r\n", bhok ? "PASS" : "FAIL");
                    g_console->vtable->write(g_console, out, (size_t)n);
                }
                else if (strcmp(line, "RTOSUSR") == 0)
                {
                    int uok = rtos_usr_selftest();
                    char out[32];
                    int n = snprintf(out, sizeof(out), "RTOSUSR %s\r\n", uok ? "PASS" : "FAIL");
                    g_console->vtable->write(g_console, out, (size_t)n);
                }
                else if (strcmp(line, "RTOSKOBJ") == 0)
                {
                    rtos_kobj_dump();
                    char out[32];
                    int n = snprintf(out, sizeof(out), "RTOSKOBJ DUMP\r\n");
                    g_console->vtable->write(g_console, out, (size_t)n);
                }
