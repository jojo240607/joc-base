...
/*
 * Hardware Abstraction Layer — General-Purpose TIMER (STM32H7 implementation).
 *
 * Same layering contract as the F1 tim_hal.h: the driver (drv/timer.c) includes
 * THIS header but NEVER sees TIM_TypeDef or any chip register; it only ever
 * handles the OPAQUE `tim_hal_handle_t *`. All register knowledge lives in
 * tim_hal.c.
 *
 ...
 * TIM15..TIM17 (GP, 2/1 channels) on APB2. Register layout is F4-compatible
 * (CR1..BDTR), so the F1 implementation ports almost verbatim.
 *
 * IRQ wiring differs from F103 (H750 vector table): TIM8 and TIM12 SHARE
 * IRQ 43 (TIM8_BRK/TIM12), TIM8 and TIM13 SHARE IRQ 44 (TIM8_UP/TIM13),
 * TIM8 and TIM14 SHARE IRQ 45 (TIM8_TRG_COM/TIM14) — the driver MUST check
 * tim_hal_uif_pending() before acting on a shared line.
 */
 ...
 * feeding this timer (board knows it: TIM1/8/15/16/17 run from the 240 MHz
 * APB2 timer clock, TIM2..7/12..14 from the 240 MHz APB1 timer clock when the
 * APB prescaler is /2); tick_hz is the desired overflow frequency. */
 ...
 * on the same line doesn't spuriously trigger their handler. On H750 the shared
 * lines are TIM8_BRK/TIM12 (IRQ43), TIM8_UP/TIM13 (IRQ44) and
 * TIM8_TRG_COM/TIM14 (IRQ45). */
 ...
 *
 * NOTE: TIM6/TIM7 (basic timers on H7) have NO CC channels — PWM is not
 * supported. tim_hal_pwm_config_channel is a no-op on those.
 * =========================================================================== */
 ...
 * No-op on TIM6/TIM7 (basic timers without CC channels). */
...
/* ===========================================================================
 * ADVANCED-TIMER features (TIM1/TIM8 on H7).
 *
 * The advanced TIM adds silicon the general-purpose TIMs lack:
...
/* Non-zero iff this handle is an advanced TIM (TIM1/TIM8 on H7). Drivers use it
 * to decide whether to touch BDTR / the complementary outputs. */
 ...
 * from the reference manual). Returns the DTG byte to write to BDTR.DTG. */
