...
/* H7 has TWO IWDGs (IWDG1/IWDG2), both in the D3 domain. Unlike the F4, they
 * are NOT in the backup domain, so no PWR DBP unlock is required — only the
 * LSI clock must be running (RCC_CSR, same bit positions as F4). */
...
{
    (void)h;
    /* The IWDG time base is the LSI (internal ~32 kHz). Start it and wait for
     * readiness — idempotent (the RTC driver may have done it already).
     * H7 IWDG1/2 are NOT in the backup domain: no DBP write is needed. */
