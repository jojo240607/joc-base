 ...
 * register-block layout of the STM32H7 family (GPIOA @ 0x58020000, each port
 ...
 * H7 GPIO registers are identical to F4 (MODER/OTYPER/OSPEEDR/PUPDR/AFR);
 * the ONLY difference is the clock gate: RCC->AHB4ENR instead of AHB1ENR.
 */
                       ...
                       const pinmux_pin_cfg_t *cfg)
{
    ...
    /* Enable the port clock. On STM32H7 the GPIOxEN bits in RCC->AHB4ENR are
     * exactly bits 0..10 for ports A..K, i.e. bit == (port index). */
    RCC->AHB4ENR |= (1U << (uint32_t)port);
