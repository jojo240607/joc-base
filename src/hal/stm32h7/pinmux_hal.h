...
/*
 * Hardware Abstraction Layer — pin multiplexing (STM32H7 implementation).
 *
 ...
 * All chip knowledge (the STM32H7 alternate-function matrix and the GPIO
 ...
 * alphabetical order. H7 has ports A..K (GPIO is on AHB4). */
typedef enum {
    PINMUX_PORT_A = 0,
    PINMUX_PORT_B,
    PINMUX_PORT_C,
 ...
 *   - Peripheral signals come from the AF database in pinmux_af_stm32h7.c,
 *     where EVERY name is UNIQUE, so the name maps to exactly one pad.
 *   - Plain GPIO pins are named "GPIO<port><pin>" (e.g. "GPIOD_12") and are
 *     parsed here directly, with no database row needed.
 ...
 * in RCC->AHB4ENR — H7 GPIO is NOT on AHB1 like F4). */
