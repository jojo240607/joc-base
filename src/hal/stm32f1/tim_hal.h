...
/*
 * Hardware Abstraction Layer — General-Purpose TIMER (STM32F1 implementation).
 *
 ...
 *
 * F103 has TIM1..TIM7. TIM1 is the only advanced timer (RCR + BDTR + complementary
 * outputs). TIM2..TIM7 are general-purpose (no RCR/BDTR). TIM2..TIM5 have 4 CC
 * channels each; TIM6/TIM7 are basic timers with no CC channels (PWM not supported).
 */
 ...
 * feeding this timer (board knows it: TIM2..TIM7 on F1 run from the 72 MHz
 * APB1 timer clock, TIM1 runs from the 72 MHz APB2 timer clock);
 * tick_hz is the desired overflow frequency. */
 ...
 *
 * NOTE: TIM6/TIM7 (basic timers on F103) have NO CC channels — PWM is not
...
/* ===========================================================================
 * ADVANCED-TIMER features (TIM1 on F103).
 *
 ...
 *     (DTG) and a Break (BRK) fault input. On TIM1 the PWM pins stay INACTIVE
 *     until MOE=1, so a PWM driver that ignores BDTR produces NO output on TIM1.
...
/* Non-zero iff this handle is an advanced TIM (TIM1 on F103). Drivers use it to
 ...
 * from the F1 reference manual). Returns the DTG byte to write to BDTR.DTG. */
