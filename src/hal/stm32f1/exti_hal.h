...
/*
 * Hardware Abstraction Layer — External Interrupt (EXTI, STM32F1 implementation).
 *
 ...
 *   - AFIO EXTICR routes the pin's PORT to the EXTI line (per pin number);
 *     F1 uses AFIO instead of F4's SYSCFG.
...
/* Route this pin's PORT to its EXTI line via AFIO EXTICR (enables AFIO clock).
 * Only ONE port may drive a given pin number — the driver must ensure no other
 * device selected a different port for the same pin.
 * NOTE: F1 uses AFIO instead of F4's SYSCFG for EXTICR. */
