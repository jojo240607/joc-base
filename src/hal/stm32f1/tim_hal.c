#include <stdlib.h>
#include <string.h>
#include <stm32f103xx.h>   /* TIM_TypeDef, RCC, IRQn_Type — board/chip layer only */
#include "tim_hal.h"

/*
 * STM32F1 general-purpose TIMER HAL.
 *
 * F103 has TIM1..TIM7. TIM1 is the only advanced timer (RCR + BDTR +
 * complementary outputs). TIM2..TIM5 are general-purpose (4 CC channels each).
 * TIM6/TIM7 are basic timers (no CC channels — PWM not supported).
 *
...
#define IS_GP_TIM(t)  ((t) >= TIM2 && (t) <= TIM7)

struct tim_hal_handle {
    TIM_TypeDef *tim;
    int advanced;   /* 1 for TIM1 (RCR + BDTR/complementary/break) */
};
...
tim_hal_handle_t *tim_hal_create(void *peripheral)
{
    ...
    /* TIM1 is the only advanced timer on F103. */
    h->advanced = (h->tim == TIM1) ? 1 : 0;
    return h;
}
...
/* F103 APB1 timers: TIM2..TIM7 (timer clock = 2xAPB1 = 72 MHz).
 * APB2 timer: TIM1 (timer clock = 2xAPB2 = 72 MHz). */
...
irq_id_t tim_hal_irq_id(tim_hal_handle_t *h)
{
    if (!h) return -1;
    TIM_TypeDef *t = h->tim;
    /* F103 dedicated IRQ lines (no shared lines like F4):
     *   TIM1_BRK=24, TIM1_UP=25, TIM1_TRG_COM=26, TIM1_CC=27,
     *   TIM2=28, TIM3=29, TIM4=30, (TIM5-7 each have dedicated lines).
     * Note: F103 does NOT map TIM6/TIM7 to shared DAC lines —
     *   TIM6_DAC_IRQn and TIM7_IRQn are separate (lines 54/55 on connectivity
     *   line devices). For the standard F103RCT6, TIM6/TIM7 do NOT have
     *   dedicated NVIC lines; they are basic timers without interrupt
     *   capability in the standard F103. For safety, return the known
     *   GP timer IRQs and -1 for TIM6/TIM7. */
    if (t == TIM1)  return (irq_id_t)TIM1_UP_IRQn;    /* use update IRQ */
    if (t == TIM2)  return (irq_id_t)TIM2_IRQn;
    if (t == TIM3)  return (irq_id_t)TIM3_IRQn;
    if (t == TIM4)  return (irq_id_t)TIM4_IRQn;
    /* TIM5 on F103 has IRQn 50 (not standard in our minimal enum) */
    if (t == TIM5)  return -1;   /* not wired in our NVIC table */
    if (t == TIM6)  return -1;   /* basic timer, no interrupt line on F103 */
    if (t == TIM7)  return -1;   /* basic timer, no interrupt line on F103 */
    return -1;
}
...
/* ===========================================================================
 * ADVANCED-TIMER features (TIM1). See tim_hal.h for the contract.
 * Every BDTR/complementary/break accessor is a no-op on a GP TIM, so the
 * drivers can call them unconditionally. TIM1 on F103 is the only advanced
 * timer — it has RCR, BDTR, DCR, DMAR registers.
 * =========================================================================== */
...
{
    if (!h || !h->advanced) return -1;     /* RCR only exists on TIM1 */
...
{
    if (!h || !h->advanced) return;        /* MOE only exists on TIM1 */
    if (on) h->tim->BDTR |=  TIM_BDTR_MOE;
    else    h->tim->BDTR &= ~TIM_BDTR_MOE;
}

/* 4-zone DTG encoding (F1 reference manual, TIMx_BDTR.DTG). Returns the 8-bit
...
void tim_hal_pwm_set_deadtime(tim_hal_handle_t *h, uint8_t dtg)
{
    ...
    h->tim->BDTR = (h->tim->BDTR & ~TIM_BDTR_DTG) | (uint32_t)dtg;
}
