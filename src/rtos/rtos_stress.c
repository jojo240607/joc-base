 ...
 *
 * 自测作为 main 任务的一部分运行；子任务在 stop 后置位后自我了结（DEAD 池槽
 * 由 rtos_task_create 回收复用，故本自测现在可重复运行）。prod/cons 用非阻塞
 * trysend/tryrecv 让出 CPU，避免测试结束时阻塞在满/空队列上造成死锁。
 * ------------------------------------------------------------------------- */
...
RTOS_SELFTEST_ADD("stress", rtos_stress_selftest);
