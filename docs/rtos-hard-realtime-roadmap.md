...
- **P2-1 调度轨迹 trace 导出** — ✅ DONE 2026-08-06（硬件实测 RTOSTRACE 导出真实切换轨迹）
  ...
  - **配套量化（Rhealstone 子集 + 硬实时延迟直方图，2026-08-07 硬件实测）**：
    ...
    - **🚀 中断延迟进一步优化（2026-08-07，定位器已落地 + 硬件实测）**：
      两步优化已落地（零延迟带封装 `irq_manager_set_zero_latency` + 临界区持锁直方图 `g_crit_hist`），
      并烧录 `build_trace` 实测。结果**推翻了初始假设**，是关键发现：

      **实测数据（STM32F407 @168MHz，RTOS_SCHED_TRACE=ON 构建烧录）**：
      - 任务切换/信号量混洗：min 199cyc(1.2µs) / avg 381cyc(2.3µs) / max 180226cyc(冷启动 1.07ms，一次性)
      - 硬实时唤醒延迟（prio≤4, 2000 样本）：min 743cyc(4.4µs) / avg 758cyc(4.5µs) / **max 5565cyc(33.1µs)**；
        1993/2000 落 4µs 桶，复现之前的长尾。
      - **临界区持锁直方图（定位器核心产出）**：total=65794，worst=**553cyc(3.3µs)**，分布：
        `[0-1µs]=32168(48.9%) [1-2µs]=26139(39.7%) [2-3µs]=5486(8.3%) [3-4µs]=2001(3.0%) [>4µs]=0`。
