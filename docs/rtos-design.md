...
- 划分**专用 BH 优先级带** `RTOS_PRIO_BH_HIGH / MED`，位于：
  ...
  - 之上还可保留极少量"零延迟 IRQ"（`RTOS_MAX_ZERO_LATENCY_IRQS`，对应 Zephyr `CONFIG_ZERO_LATENCY_IRQS` / FreeRTOS `configMAX_SYSCALL_INTERRUPT_PRIORITY`），这类 ISR 不被内核屏蔽，用于处理抖动最敏感的硬件。**已实现**：`RTOS_MAX_ZERO_LATENCY_IRQS` 配置 + 内核临界区统一入口 `rtos_crit_enter/exit`（`core/rtos_internal.h`）——该值 `>0` 时改用 `BASEPRI` 仅屏蔽优先级 `>=` 阈值的异常（高于阈值的零延迟 ISR 永不被屏蔽），`=0`（默认）退化为 `PRIMASK` 全局关中断，行为与此前完全一致、零回归；`rtos_start()` 在启用时会把 SysTick/PendSV 置于可被屏蔽的优先级带。启用须遵守 FreeRTOS 式契约：所有调用内核 API 的 ISR 优先级必须 `>=` 该阈值（见 `src/rtos/include/rtos_config.h` 注释）。**上下文切换窗口也已收口到 `rtos_pendsv_switch` 内走 `rtos_crit_enter/exit`**（`context.S` 的 `PendSV_Handler` 不再裸用 `cpsid i`/`cpsie i`），故 `ZERO_LATENCY>0` 时连“选下一任务”这一小段也只对内核 ISR 屏蔽、零延迟 ISR 在切换期间仍可达，避免任务切换粗暴关全中断。
  - **集中式优先级治理（已落地）**：中断"使能/去使能"本就由 `irq_manager` 统一管理（引用计数 + "enabled⇒有回调" 防空槽锁死 + `irq_manager_dump` 审计）；但优先级**分配**之前是各驱动散落的魔法数字。现补上统一治理：**(1)** `irq.h` 定义语义带 `IRQ_PRIO_KERNEL`(5)/`IRQ_PRIO_ZERO_LATENCY`(2)/`IRQ_PRIO_DEFAULT`(8) 与 ISR 类别 `IRQ_CLASS_KERNEL`/`IRQ_CLASS_ZERO_LATENCY`/`IRQ_CLASS_NORMAL`；**(2)** 各驱动改用 `irq_manager_set_priority(id, 带, 类别)` 设置优先级，管理器把 `prio`/`cls` 记入 `g_mgr[]`；**(3)** `irq_manager_dump()` 现打印每路的 `prio`/`cls`；**(4)** `rtos_start()` 启动期调用 `irq_manager_audit_priorities(RTOS_MAX_ZERO_LATENCY_IRQS)` 校验 FreeRTOS 式契约——任何 `IRQ_CLASS_KERNEL` 的 ISR 优先级 `<` 阈值、或 `IRQ_CLASS_ZERO_LATENCY` 的 ISR 优先级 `>=` 阈值都会逐个打印违例。默认 `ZERO_LATENCY=0` 时审计不报任何违例、行为与此前逐字节一致。
- **延迟预算**：
